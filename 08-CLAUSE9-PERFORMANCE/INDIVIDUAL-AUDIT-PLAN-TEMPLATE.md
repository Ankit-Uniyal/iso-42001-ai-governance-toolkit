# Individual Audit Plan
## ISO/IEC 42001:2023 | Clause 9.2 — Template

**Audit Reference:** AUD-[###]-[YEAR]
**Audit Area:** ___________________________
**ISO 42001 Clauses:** ___________________________
**Audit Type:** Full AIMS / Clause-specific / AI System Audit
**Lead Auditor:** ___________________________
**Auditees:** ___________________________
**Programme Reference:** AIMS-AUDITPROG-[YEAR]

---

## 1. Objectives

1. Whether [area] conforms to ISO/IEC 42001:2023 requirements
2. Whether [area] conforms to the organisation's AIMS requirements
3. Whether [area] is effectively implemented and maintained

---

## 2. Scope

**In Scope:** [Specific clauses, processes, AI systems, activities]

**Out of Scope:** [Excluded areas and justification]

**Sampling Approach:** [e.g., "All High-risk AI systems"; "Random sample of 3 recent Impact Assessments"]

---

## 3. Schedule

| Date | Time | Activity | Location | Auditee(s) |
|------|------|---------|---------|-----------|
| | | Opening Meeting | | AI Gov Lead, team |
| | | Document Review | Remote | Lead Auditor |
| | | Interviews | | [Names/Roles] |
| | | Process Observation | | [Names/Roles] |
| | | Auditor Caucus | | Lead Auditor |
| | | Closing Meeting | | AI Gov Lead, team |

---

## 4. Methods

| Method | What Will Be Reviewed |
|--------|----------------------|
| Document review | [e.g., AI Policy, Risk Register, Training Records] |
| Interviews | [Specific roles] |
| Process observation | [Specific processes] |
| Record sampling | [e.g., last 3 Impact Assessments] |

---

## 5. Audit Criteria

1. ISO/IEC 42001:2023 — [specific clauses]
2. Organisation's AIMS policies and procedures
3. Applicable regulatory requirements

---

## 6. Documents Requested

| Document | Doc ID | Purpose |
|---------|--------|---------|
| AI Policy | AIMS-POLICY-001 | Verify currency |
| AI Risk Register | AI-RISK-REGISTER | Verify coverage |
| | | |

---

## 7. Independence Statement

I confirm no conflict of interest with areas being audited.

**Lead Auditor:** ___________________________ **Date:** ___________________________

---

## 8. Report Reference

- Findings reported in: AUD-[###]-[YEAR]-REPORT
- NCRs raised in: NCR-REGISTER.md

---

*ISO/IEC 42001:2023 AI Governance Toolkit | Clause 9.2 | See root README.md for full index*
